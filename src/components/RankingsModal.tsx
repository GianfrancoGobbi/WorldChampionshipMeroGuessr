...
const RankingsModal: React.FC<RankingsModalProps> = ({ isOpen, onClose, user }) => {
    ...
    }, [isOpen, period, activeTab, loadRankings, loadPlayedRounds]);
    ...
    return (
        <>
        <div className={`fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[52] p-4 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={onClose}>
            <div className="bg-gray-800 text-white p-6 rounded-xl shadow-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
